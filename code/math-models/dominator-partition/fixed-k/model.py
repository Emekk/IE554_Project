...
def create_and_solve_model(
    ...
    for i in PI.difference({K}):
        m.addConstr(
            gp.quicksum(x[v, i] for v in V) >= gp.quicksum(x[v, i + 1] for v in V),
            name=f"Order_{i}",
        )

    # VALID INEQUALITIES
    m.addConstr(gp.quicksum(x[v, 1] for v in V) >= np.ceil(len(V) / K), name="Valid1")
    for i in PI:
        m.addConstr(
            gp.quicksum(x[v, i] for v in V) <= np.floor((len(V) - K + i) / i),
            name=f"Valid2_{i}",
        )
    for v in V:
        for i in PI:
            m.addConstr(
                d[v, i] >= gp.quicksum(x[u, i] for u in CN[v]) - (len(CN[v]) - 1),
                name=f"Valid3_{v}_{i}",
            )
    for v in V:
        for i in PI:
            m.addConstr(
                d[v, i] <= gp.quicksum(x[u, i] for u in CN[v]), name=f"Valid4_{v}_{i}"
            )
    for MIS in MAXIMAL_INDEPENDENT_SETS:
        for i in PI:
            m.addConstr(gp.quicksum(d[v, i] for v in MIS) <= 1, name=f"Valid5_{i}")
    for i in PI:
        for v in V:
            for MIS in MAXIMAL_INDEPENDENT_SETS:
                if v in MIS:
                    m.addConstr(
                        gp.quicksum(x[u, i] for u in CN[v])
                        <= len(CN[v])
                        - 1
                        + d[v, i]
                        - gp.quicksum(d[y, i] for y in MIS if y != v),
                        name=f"Valid6_{v}_{i}",
                    )
    if len(V) / K <= 2:
        for i in PI.difference({1}):
...
def display_results(
    m, x, d, V, E, PI, search_feasible, save_path, graph_path=None, partitioning=False
):
    partitions = []
...
def evaluate_function_at_all_solutions(m, x, d, V, PI, coef_x, coef_d):
    ...
    if m.SolCount > 0:
        for solNum in range(m.SolCount):
            m.setParam(GRB.Param.SolutionNumber, solNum)
            value = sum(coef_x[v - 1, i - 1] * x[v, i].Xn for v in V for i in PI) + sum(
                coef_d[v - 1, i - 1] * d[v, i].Xn for v in V for i in PI
            )
            if value < min_value:
                min_value = value
            if value > max_value:
